...
db = client.EEG  # Replace 'your-database' with your actual database name
...
def get_image(image_id):
    # Find the image data by its ID or filename in the MongoDB collection
    image_data = collection.find_one({'filename': image_id})

    if image_data:
        # You may need to set the appropriate content type based on the image file type (e.g., 'image/png', 'image/jpeg')
        response = send_file(os.path.join('uploads', image_data['filename']), as_attachment=False)
        return response
    else:
